...
if __name__ == "__main__":
    ...
    remake_location = (
        pathlib.Path(__file__).parents[2].joinpath("remake_db.sh")
    )
    sql_scripts_location = (
        pathlib.Path(__file__).parents[2].joinpath("sql_scripts")
    )
    mysql_location = pathlib.Path.home().joinpath(
        "bd2_23L_z09_mysql/mysql/bin/mysql"
    )
            ...
            if len(line) > 1:
                ...
                if len(out) > 0:
                    ...
                    result_lines = [
                        re.sub(
                            r"(\.\d*?)0+([^\d]|$)",
                            lambda matched: matched.group(1)
                            + matched.group(2),
                            line,
                        )
                    ...
                    print(
                        *[
                            "\033[34;1mDBMS OUTPUT:\033[0m " + result_line
